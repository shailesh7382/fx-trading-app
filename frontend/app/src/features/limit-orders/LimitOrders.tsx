...
import { extractApiMessage, fetchLimitOrders } from '@/shared/api/client';
import { useWorkspaceContext } from '@/features/workspace/useWorkspaceData';
import type { LimitOrder, LimitOrderStatus } from '@/shared/types';
import { formatDateTime, formatNotional, formatRate } from '@/shared/utils/formatters';

const statusOptions = ['ALL', 'ACTIVE', 'EXECUTED', 'EXPIRED', 'CANCELLED'] as const;
...
const statusPresentation: Record<LimitOrderStatus, StatusPresentation> = {
...
function StatusChip({ status }: { status: LimitOrderStatus }) {
...
function OrderCell({ label, children, sx = {} }: OrderCellProps) {
  return (
