package com.example.fx.frontend.web;
...
public class ApiProxyController {
    ...
    @Value("${app.proxy.backend-base-url:http://localhost:8080}")
    private String backendBaseUrl;

    @RequestMapping({"/api", "/api/**"})
    public ResponseEntity<byte[]> proxy(HttpServletRequest request) throws IOException, InterruptedException {
        UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromUriString(backendBaseUrl)
