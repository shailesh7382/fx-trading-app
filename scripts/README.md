...
starting, inspecting, and stopping the local FX trading stack. Run them from
...
- `stop-full-stack-prod.sh` and `status-full-stack-prod.sh` — compatibility
  aliases for the mode-independent stop and status commands.

## Build and operations

```bash
scripts/build-package-full-stack.sh
scripts/status-full-stack.sh
...
- Start and status flows: Bash with `/dev/tcp` support.
...
| --- | --- | --- |
| `STARTUP_TIMEOUT_SECONDS` | `120` | Per-service readiness timeout |
| `PORT_CHECK_HOST` | `127.0.0.1` | Host used for local TCP readiness checks |
...
and `STOP_PORT_CHECK_TIMEOUT_SECONDS` (default `1`). External process and socket
inspection is intentionally isolated in that command.
...
Each start creates `logs/runs/<timestamp>-<pid>/` and updates `logs/current`.
Per-service logs and generated launchers live in the run directory. Managed PID
and metadata files live under `.runtime/`.

Startup is transactional: if a later service fails readiness, services started
by that attempt are stopped in reverse order. Stop operations verify the stored
process marker before sending signals and leave untracked listeners untouched.
